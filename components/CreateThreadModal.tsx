...
const CreateThreadModal: React.FC<CreateThreadModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-brand-surface rounded-2xl shadow-2xl w-full max-w-2xl border border-brand-border" onClick={e => e.stopPropagation()}>
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="space-y-4">
             <div>
                <label htmlFor="thread-title" className="block text-sm font-medium text-brand-text-secondary mb-1">Заголовок</label>
                <input
                  id="thread-title"
                  type="text"
                  value={title}
               ...
               <div>
                <label htmlFor="thread-content" className="block text-sm font-medium text-brand-text-secondary mb-1">Ваше сообщение</label>
                <textarea
                  id="thread-content"
                  rows={8}
                  value={content}
          ...
          <div className="mt-6 flex justify-end gap-4">
             <button
                ...
                onClick={onClose}
                disabled={isSubmitting}
                className="bg-brand-border hover:bg-brand-border/80 text-white font-bold py-2 px-6 rounded-lg transition-colors"
              >
                Отмена
              </button>
            <button
              type="submit"
              disabled={isSubmitting}
              className="bg-brand-primary hover:bg-brand-primary-hover text-white font-bold py-2 px-6 rounded-lg transition-colors flex items-center justify-center min-w-[150px]"
            >
