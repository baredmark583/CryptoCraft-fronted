    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-brand-surface rounded-2xl shadow-2xl w-full max-w-md border border-brand-border" onClick={e => e.stopPropagation()}>
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <p className="text-brand-text-secondary mb-4">Текущий баланс: <span className="font-bold text-green-400">{user.balance.toFixed(2)} USDT</span></p>

          <div>
             <label htmlFor="amount" className="block text-sm font-medium text-brand-text-secondary mb-2">Сумма (USDT)</label>
             <input 
                id="amount"
                type="number"
                value={amount}
           ...
           <button
            ...
            className="w-full mt-6 bg-brand-primary hover:bg-brand-primary-hover text-white font-bold py-3 rounded-lg flex items-center justify-center disabled:bg-gray-500"
          >
