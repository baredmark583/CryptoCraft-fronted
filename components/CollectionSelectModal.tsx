...
const CollectionSelectModal: React.FC<CollectionSelectModalProps> = ({ productId, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-brand-surface rounded-2xl shadow-2xl w-full max-w-sm border border-brand-border" onClick={e => e.stopPropagation()}>
                <div className="p-6">
                    <div className="flex justify-between items-center mb-4">
                        <h2 className="text-xl font-bold text-white">Сохранить в...</h2>
                        <button onClick={onClose} className="text-brand-text-secondary hover:text-white text-3xl leading-none">&times;</button>
                    </div>

                    <div className="max-h-60 overflow-y-auto space-y-2 pr-2 mb-4">
                        {collections.map(collection => (
                             <label key={collection.id} className="flex items-center space-x-3 p-3 bg-brand-background rounded-lg cursor-pointer hover:bg-brand-border transition-colors">
                                <input 
                                    type="checkbox" 
                    ...
                    <form onSubmit={handleCreateAndAdd}>
                        <label className="block text-sm font-medium text-brand-text-secondary mb-2">Создать новую коллекцию</label>
                        <div className="flex gap-2">
                            <input
                                ...
                                className="flex-grow bg-brand-background border border-brand-border rounded-md shadow-sm p-2"
                            />
                            <button
                                type="submit"
                                disabled={isCreating || !newCollectionName.trim()}
                                className="px-4 py-2 bg-brand-primary hover:bg-brand-primary-hover text-white font-bold rounded-lg transition-colors flex items-center justify-center disabled:bg-gray-500"
                            >
                 ...
                 <div className="bg-brand-background/50 p-3 text-center">
                    <button onClick={onClose} className="font-bold text-brand-primary hover:text-brand-primary-hover">Готово</button>
                </div>
            </div>
        </div>
    );
