    ...
    return (
        ...
        >
            ...
            >
                ...
                <div className="relative bg-brand-background rounded-xl p-6">
                    ...
                    <div className="aspect-square bg-brand-surface rounded-lg overflow-hidden mb-4 border-2 border-brand-border">
                        <img src={product.imageUrls[0]} alt={product.title} className="w-full h-full object-cover" />
                    </div>

                    <div className="text-center mb-4">
                        <h3 className="text-xl font-bold text-white">{product.title}</h3>
                        <p className="text-sm text-brand-text-secondary">от <span className="font-semibold text-brand-text-primary">{product.seller.name}</span></p>
                    </div>

                    <div className="space-y-3 text-sm border-t border-b border-brand-border/50 py-4">
                        <div className="flex justify-between">
                            <span className="text-brand-text-secondary">Дата проверки:</span>
                            <span className="font-mono text-white">{new Date().toLocaleDateString()}</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-brand-text-secondary">Token ID:</span>
                            <span className="font-mono text-white truncate">{product.nftTokenId}</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-brand-text-secondary">Адрес контракта:</span>
