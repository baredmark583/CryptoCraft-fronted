...
## Админ-панель: Центр Управления CryptoCraft

Админ-панель — это отдельное React-приложение, предназначенное для полного управления, модерации и анализа маркетплейса. Она предоставляет мощные инструменты для администраторов и модераторов.

### Архитектура

-   **Технологии:** React, TypeScript, TailwindCSS, React Router.
-   **Развертывание:** Как отдельный статический сайт для максимальной безопасности, производительности и независимого развертывания.
-   **API:** Взаимодействует с бэкендом NestJS через специальный, защищенный набор эндпоинтов (`/api/v1/admin/...`).

### Mock API (`adminApiService.ts`)

На текущем этапе разработки вся функциональность админ-панели симулируется с помощью mock-сервиса (`admin/services/adminApiService.ts`). Это позволяет полностью разработать и протестировать интерфейс без прямой зависимости от бэкенда. Сервис содержит mock-данные и функции для имитации CRUD-операций.

### Ключевые Модули и Функциональность

#### 1. **Главная Панель (Dashboard) — ✅ (ЗАВЕРШЕНО)**
-   Мощный центр аналитики с ключевыми метриками (KPIs):
    -   Общий оборот (USDT) и прибыль платформы.
    -   Количество новых пользователей.
    -   Товары на модерации и активные споры.
-   Визуализация данных с помощью графиков (продажи по дням, по категориям).

#### 2. **Управление Пользователями и Администраторами — ✅ (ЗАВЕРШЕНО)**
-   **CRUD Пользователей:** Реализован интерфейс для просмотра, поиска, фильтрации и редактирования всех пользователей.
-   **Модерация:** Реализована функция блокировки/разблокировки пользователей.
-   **Статусы:** Реализовано изменение Pro-статуса.
-   **Финансы:** Реализовано редактирование баланса пользователя.
-   **Управление Администраторами (в плане):**
    -   Супер-админ создается через `.env` на бэкенде.
    -   Супер-админ может назначать модераторов из числа пользователей, задавать им временный пароль.
    -   Гибкая настройка прав доступа для модераторов.

#### 3. **Управление Контентом и Товарами — ✅ (ЗАВЕРШЕНО)**
-   **Модерация Товаров:** Реализована отдельная страница-очередь для объявлений, ожидающих одобрения или отклонения с указанием причины.
-   **Управление Категориями:** Реализован интерфейс для динамического создания и редактирования категорий и их специфических атрибутов (полностью заменяет `constants.ts`).
-   **Библиотека Иконок:** Реализована страница для централизованного управления иконками (SVG-код), позволяющая задавать для каждой иконки индивидуальный размер (ширину и высоту в пикселях), для всего пользовательского фронтенда.

#### 4. **Заказы и Споры**
-   **Просмотр заказов — ✅ (ЗАВЕРШЕНО):** Реализован интерфейс для просмотра и управления всеми заказами в системе.
-   **Центр Разрешения Споров — ✅ (ЗАВЕРШЕНО):** Выделенный интерфейс для арбитража, где администратор видит переписку и выносит решение.

#### 5. **Финансы и Маркетинг**
-   **Просмотр Транзакций — ✅ (ЗАВЕРШЕНО):** Просмотр лога всех финансовых операций.
-   **Промокоды — ✅ (ЗАВЕРШЕНО):** Создание "общих" промокодов от имени платформы.

#### 6. **Настройки Платформы — (в процессе)**
-   **API Ключи — ✅ (ЗАВЕРШЕНО):** Безопасный интерфейс для управления ключами внешних сервисов.
-   **Кастомизация Фронтенда (в плане):** Уникальный инструмент для изменения внешнего вида пользовательского сайта без написания кода.
-   **Live-трансляции (в плане):** Возможность проведения персональных трансляций от имени администратора.

#### 7. **Отладка — ✅ (ЗАВЕРШЕНО)**
-   **Debug-меню:** Реализована встроенная консоль, которая в реальном времени отображает логи с бэкенда через WebSocket-соединение (симуляция), устраняя необходимость прямого доступа к серверному терминалу.