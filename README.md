...
#### 2. **Управление Пользователями и Администраторами**
-   **CRUD Пользователей:** Просмотр, поиск, фильтрация и редактирование всех пользователей.
-   **Модерация:** Блокировка/разблокировка пользователей.
-   **Статусы:** Выдача/отзыв Pro-статуса.
-   **Финансы:** Начисление/списание средств с баланса, выдача бонусов.
-   **Управление Администраторами:**
    ...
    -   Гибкая настройка прав доступа для модераторов (например, доступ только к определенным категориям товаров).

#### 3. **Управление Контентом и Товарами**
-   **Модерация Товаров:** Очередь объявлений для одобрения или отклонения с указанием причины.
-   **Управление Категориями:** Динамическое создание и редактирование категорий и подкатегорий, включая их специфические атрибуты (полностью заменяет `constants.ts`).
-   **Библиотека Иконок:**
    -   Централизованное управление иконками для всего пользовательского фронтенда.
    -   Поддержка двух режимов: загрузка SVG-файла на сервер и указание ссылки на иконку (например, с `iconify.design`).

#### 4. **Заказы и Споры**
-   Просмотр всех заказов в системе.
-   **Центр Разрешения Споров:** Выделенный интерфейс для арбитража, где администратор видит переписку и выносит решение.

#### 5. **Финансы и Маркетинг**
-   **Промокоды:** Создание "общих" промокодов от имени платформы.
-   **Транзакции:** Просмотр лога всех финансовых операций.

#### 6. **Настройки Платформы**
-   **API Ключи:** Безопасный интерфейс для управления ключами внешних сервисов (почтовые службы, платежные системы).
-   **Кастомизация Фронтенда:** Уникальный инструмент для изменения внешнего вида пользовательского сайта без написания кода:
    ...
    -   Изменение радиусов скругления элементов.
    -   Настройки применяются динамически, загружаясь с бэкенда.
-   **Live-трансляции:** Возможность проведения персональных трансляций от имени администратора.

#### 7. **Отладка**
-   **Debug-меню:** Встроенная консоль, которая в реальном времени отображает логи с бэкенда через WebSocket-соединение, устраняя необходимость прямого доступа к серверному терминалу.