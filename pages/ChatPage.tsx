...
const ChatPage: React.FC = () => {
  ...
  return (
    <div className="flex h-[calc(100vh-10rem)] bg-brand-surface sm:rounded-lg sm:shadow-xl overflow-hidden">
      {/* Chat List (hidden on mobile when a chat is open) */}
      <div className={`w-full sm:w-1/3 border-r border-brand-border overflow-y-auto ${chatId ? 'hidden sm:block' : 'block'}`}>
        ...
        {chats.length > 0 ? (
            ...
            {chats.map(chat => (
                ...
                className={`p-4 flex items-center cursor-pointer transition-colors ${selectedChat?.id === chat.id ? 'bg-brand-primary/20' : 'hover:bg-brand-border/50'}`}>
                ...
                <div className="flex-grow overflow-hidden">
                    ...
                    <p className="text-sm text-brand-text-secondary truncate">{chat.lastMessage.text}</p>
                </div>
                </li>
            ))}
            </ul>
        ) : (
            <div className="p-4 text-center text-brand-text-secondary">У вас пока нет чатов.</div>
        )}
      </div>
        ...
        {selectedChat ? (
          <>
            <div className="p-4 border-b border-brand-border flex items-center">
                <button onClick={() => navigate('/chat')} className="sm:hidden mr-2 p-1 text-brand-text-secondary hover:text-white">
            ...
            <div className="flex-grow p-6 overflow-y-auto bg-brand-background">
              {selectedChat.messages.length > 0 ? selectedChat.messages.map(message => (
                <ChatMessage key={message.id} message={message} isOwnMessage={message.senderId === user.id} onQuickReplyClick={handleQuickReplyClick} />
                 ...
                 <div className="text-center text-brand-text-secondary">Сообщений пока нет.</div>
              )}
            ...
            <div className="p-4 bg-brand-surface">
              <form onSubmit={handleSendTextMessage} className="flex items-center space-x-2">
                ...
                <input
                  ...
                  className="flex-grow bg-brand-background border border-brand-border rounded-full py-3 px-4 focus:outline-none focus:ring-2 focus:ring-brand-primary"
                />
                <button type="submit" className="bg-brand-primary text-white rounded-full p-3 hover:bg-brand-primary-hover transition-colors">
          ...
          <div className="flex-grow flex items-center justify-center bg-brand-background">
            <p className="text-brand-text-secondary">Выберите чат, чтобы начать общение</p>
          </div>
        )}
      </div>
    </div>
