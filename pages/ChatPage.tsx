...
const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on('newMessage', (newMessage: Message) => {
        setSelectedChat(prevChat => {
            // FIX: Add null check for newMessage.chat as it is an optional property.
            if (prevChat && newMessage.chat && prevChat.id === newMessage.chat.id) {
                // Remove system messages (quick replies) before adding new one
  ...
  return (
    <div className="flex h-screen bg-white text-gray-900 font-sans">
      ...
      {/* Message Area */}
      <div className={`w-full md:flex-1 flex-col ${chatId ? 'flex' : 'hidden md:flex'}`}>
        {selectedChat ? (
          <>
            ...
            <div className="p-2 sm:p-4 bg-gray-100 border-t border-gray-200">
              <form onSubmit={handleSendTextMessage} className="flex items-center space-x-2">
                ...
                >
                  <DynamicIcon name="attachment-clip" className="w-6 h-6" fallback={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M18.375 12.739l-7.693 7.693a4.5 4.5 0 01-6.364-6.364l10.94-10.94A3 3 0 1119.5 7.372L8.552 18.32m.009-.01l-.01.01.01-.01z"/></svg>}/>
                </button>
                <input
                  type="text"
                  value={messageText}
                ...
                >
                  <DynamicIcon name="send-arrow" className="w-5 h-5" fallback={<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path d="M3.105 2.289a.75.75 0 00-.826.95l1.414 4.949a.75.75 0 00.95.54l3.123-.93a.75.75 0 01.928.928l-.93 3.123a.75.75 0 00.54.95l4.95 1.414a.75.75 0 00.95-.826l-2.434-8.518a.75.75 0 00-.702-.556l-8.518-2.434z" /></svg>}/>
                </button>
              </form>
            </div>
          </>
