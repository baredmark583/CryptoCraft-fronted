...
const ProposalDetailPage: React.FC = () => {
    ...
    return (
        <div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                    <div>
                     ...
                     {proposal.status === 'ACTIVE' && (
                         <div className="text-center bg-base-100 p-3 rounded-lg flex-shrink-0 w-full sm:w-auto">
            ...
            {/* * ГЛАВНОЕ ИЗМЕНЕНИЕ:
              * Добавлены классы `order-` для изменения порядка на мобильных.
              * `order-1 lg:order-2` для сайдбара (результаты, голосование)
              * `order-2 lg:order-1` для описания
            */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-base-100 p-6 rounded-lg order-2 lg:order-1">
                ...
                <div className="lg:col-span-1 space-y-6 order-1 lg:order-2">
                    <div className="bg-base-100 p-6 rounded-lg">
                        ...
                        {/* * ИЗМЕНЕНИЕ PROGRESS BAR:
                          * Добавлен `flex` к контейнеру.
                          * Убраны `rounded-l-full`, `rounded-r-full`, `marginLeft` и `marginTop` у дочерних элементов.
                          * Родительский `overflow-hidden` и `rounded-full` сам сделает красивые края.
                        */}
                       <div className="w-full bg-base-200 rounded-full h-2.5 mt-3 overflow-hidden flex">
                           <div className="bg-green-500 h-2.5" style={{ width: `${forPercentage}%` }}></div>
                           <div className="bg-red-500 h-2.5" style={{ width: `${againstPercentage}%` }}></div>
                       </div>
