...
const CountdownTimer: React.FC<{ endDate: number }> = ({ endDate }) => {
    ...
    return <span className="font-mono text-brand-primary">{`${days}д ${hours}ч ${minutes}м ${seconds}с`}</span>;
};
...
const ProposalDetailPage: React.FC = () => {
    ...
    if (!proposal) {
        return <div className="text-center text-2xl text-brand-text-secondary mt-16">Предложение не найдено</div>;
    }
    ...
    if (user.verificationLevel !== 'PRO') {
         return (
             <div className="text-center py-20 bg-brand-surface rounded-lg">
                ...
                <p className="text-brand-text-secondary mb-8">Просмотр и голосование по предложениям доступны только для пользователей со статусом PRO.</p>
    ...
    return (
        <div>
            <div className="mb-8">
                <Link to="/governance" className="text-sm text-brand-secondary hover:text-brand-primary mb-4 block">&larr; Вернуться ко всем предложениям</Link>
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                    <div>
                        ...
                        <div className="flex items-center gap-2 text-sm text-brand-text-secondary mt-2">
                     ...
                     {proposal.status === 'ACTIVE' && (
                        <div className="text-center bg-brand-surface p-3 rounded-lg flex-shrink-0">
                             <p className="text-xs text-brand-text-secondary">Осталось времени:</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-brand-surface p-6 rounded-lg">
                    ...
                    <p className="text-brand-text-primary whitespace-pre-wrap leading-relaxed">{proposal.description}</p>
                </div>
                <div className="lg:col-span-1 space-y-6">
                     <div className="bg-brand-surface p-6 rounded-lg">
                         ...
                         <div className="w-full bg-brand-background rounded-full h-2.5 mt-3 overflow-hidden">
                        ...
                        <p className="text-xs text-brand-text-secondary text-center mt-2">Всего голосов: {totalVotes.toLocaleString()}</p>
                    </div>

                     <div className="bg-brand-surface p-6 rounded-lg">
                         <h2 className="text-xl font-bold text-white mb-4">Ваш голос</h2>
                         {proposal.status !== 'ACTIVE' ? (
                              <p className="text-brand-text-secondary text-center">Голосование по этому предложению завершено.</p>
                         ) : userVote ? (
